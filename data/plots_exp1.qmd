...
data_dir <- paste0(data_dir, '/exp_1/')
...
```{r}
# convert ratings so they go from 0 to 1 instead of -200 to 200
...
for (emotion in emotion_terms) {
  ...
  p <- ggplot(emotion_data, aes(x = avg_rating, y = mean_response)) +
    ...
    scale_x_continuous(limits = c(0, 1)) +   # force x-axis 0–1
    scale_y_continuous(limits = c(0, 1))     # force y-axis 0–1
...
#correlations grouped by emotion term and colorscale

```{r}
library(tidyverse)

# Compute correlations per emotion across all colorscales
cor_results <- merged_data %>%
  group_by(concept) %>%
  summarise(correlation = cor(avg_rating, mean_response, use = "complete.obs"),
            .groups = "drop")

# wide format
cor_results_wide <- cor_results %>%
  pivot_wider(names_from = concept, values_from = correlation)

print(cor_results_wide)
