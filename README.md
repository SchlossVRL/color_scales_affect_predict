...
Corresponding analysis notebooks in `.qmd` format for each experiment can be found in the analysis directory. Analysis for experiment 1 included `color_space_regressions_hb.R` and `predicted_ratings_averages.R` which were used to get the predicted emotion ratings for each of the colorscales used. 
Analysis for experiment 3 was combined in a single analysis file exp3.qmd. Additionally, there is included `rm_ids.R`, `rm_ids_combine.R`, and `Preprocessing.R` used to remove the subjecty pool ids or participants and prep ratings data for analysis. 
